...
원본 PDF는 텍스트 레이어가 없는 벡터 렌더링 문서라 `pypdf`/`pymupdf`로 직접 추출이 되지 않았으며, 위 마크다운 변환본을 근거 원문으로 채택했습니다. 생활관규정과 학생상벌에관한규정(징계 절차 세부사항)은 원문 확보가 되지 않아 이번 범위에서 완전히 제외했습니다 — 관련 문의는 OTHER로 분류해 담당 부서로 이관합니다.
...
| STUDENT_LIFE | 학생생활규정 전문 | 생활관·학생상벌에관한규정은 문서 미확보로 완전히 제외 |
...
| 지표 | 값 |
| --- | --- |
| 라우팅 정확도 (eval 60건) | 0.883 |
| 라우팅 macro F1 (4개 카테고리) | 0.906 |
