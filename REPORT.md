...
가드레일은 답변 속 숫자를 정규식으로 추출해 근거(도구 결과 + 카테고리 컨텍스트) 안에 문자열로 존재하는지 확인하는 방식입니다. 짧은 숫자(예: "5", "10")는 문서 전체에 우연히 등장할 수 있어 오탐을 놓치는 한계가 있습니다. 또한 도구가 준 숫자를 다른 표현으로 재계산(예: "5/6"을 "83.33%"로 환산)하면 정당한 답변도 근거 불일치로 걸러질 수 있어, "도구가 준 숫자·비율을 그대로 쓰라"고 프롬프트에 명시해 이런 과탐을 방지합니다.

### 4.5 병렬 처리 검증

`common.py`의 `pmap()`(ThreadPoolExecutor 기반)이 실제로 병렬화되는지 측정했습니다. 동일한 LLM 호출 8건을 `workers=1`(직렬)과 `workers=8`(병렬)로 각각 실행한 결과, 13.71초 → 1.23초로 약 11배 단축을 확인했습니다. `evaluate.py`의 3개 `pmap()` 호출 지점은 모두 `config.WORKERS`를 전달받아 동작합니다.
...
- **멀티턴**: `InMemorySaver` 체크포인터로 `thread_id`별 대화 상태를 유지하되, 각 턴의 라우팅·답변 생성은 직전 턴의 질문을 이어붙이지 않고 현재 질문만으로 독립 판단합니다 — 대화 맥락과 무관하게 매 턴을 독립적으로 분류·답변해 이전 주제가 새 질문에 섞이지 않습니다.

---

## 6. 데모 설계

Gradio 4-tab 구성으로 `python app.py` 실행 후 `http://127.0.0.1:7861`에서 접속합니다.
...
무엇을 왜 바꿨고 수치가 어떻게 변했는지 회차별로 `experiment_log.csv`에 남기는 탭입니다. 입력 폼 없이 순수 열람 전용 표로 구성했으며(회차를 추가하려면 `add_log_entry()`를 직접 호출), 라우팅 정확도·macro F1·답변 통과율은 직전 회차 대비 증감을 함께 표시하고, 오분류·범위밖 인식 건수는 "성과 및 오답 메모" 열에 자동 요약됩니다. §4.2의 개선 기록 표는 이 로그를 근거로 작성했습니다.

### 6.4 📋 라우트 및 업무 규정

에이전트가 실제로 쓰는 라우팅 분류 기준(`ROUTE_GUIDE`)과 답변 생성 규칙(`ANSWER_RULES`), 카테고리별 근거 문서 매핑(`policy_academic.md`)을 코드 원문 그대로 보여주는 탭입니다. 라우팅 결과가 이상해 보일 때 프롬프트 자체를 바로 확인할 수 있도록 만들었습니다.
...
![개선 기록](screenshots/experiment_log.png)
...
- **남은 과제**: 답변 품질 개선(그라운딩·문장 구성 등 답변 자체의 완성도를 높이는 작업)을 우선 진행하고, 그다음으로 입학처 관련 근거 문서를 추가해 안내 범위를 넓히는 것이 다음 과제입니다.
